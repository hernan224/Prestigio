...
class gallery_ss{
    static function init() {
        ...
        remove_shortcode('gallery', 'gallery_shortcode');  /** Remueve el shortcode gallery **/
        add_shortcode( 'gallery', array(__CLASS__, 'gss_shortcode') );  /** Redefine el shortcode gallery **/
        //add_shortcode( 'gss', array(__CLASS__, 'gss_shortcode') );  /** Shortcode agregado, no reemplaza al shortocode gallery **/
        add_action( 'wp_enqueue_scripts', array(__CLASS__, 'gss_enqueue_scripts') );
    }

    static function gss_shortcode($atts) {
        extract( shortcode_atts( array( 'ids' => '', 'name' => 'gslideshow', 'style' => '', 'image_size' => 'large' ), $atts ) );
        $ids = explode( ',', $ids );
        ...
        foreach( $ids as $image_id ){
            ...
            $version_reducida_url = wp_get_attachment_image_src( $attachment['ID'], $image_size, false );
            $slides .= "\t\t" . '<a href="'. $attachment['guid'] .'" title="'. htmlspecialchars($attachment['post_excerpt']) .'"><img src="' . $version_reducida_url[0] . '" alt="' . htmlspecialchars($attachment['post_excerpt']) . '" /></a>' . "\n";
